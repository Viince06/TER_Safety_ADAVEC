...
#ifdef __cplusplus
  #ifdef pSimArrayClock4VTable_defined
    extern struct SimTypeVTable *pSimArrayClock4VTable;
  #else 
    struct SimTypeVTable *pSimArrayClock4VTable = NULL;
  #endif
#else
  struct SimTypeVTable *pSimArrayClock4VTable;
#endif

int ArrayClock4_to_string(const void *pValue, PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    if (pSimArrayClock4VTable != NULL
        && pSimArrayClock4VTable->m_pfnGetConvInfo(SptString, SptNone) == 1) {
       return pfnStrAppend(*(char**)pSimArrayClock4VTable->m_pfnToType(SptString, pValue), pStrObj);
    }
    return pConverter->m_pfnArrayToString(pValue, ClockStatus_to_string, 4, sizeof(ClockStatus), pfnStrAppend, pStrObj);
}

int check_ArrayClock4_string(const char *str, char **endptr)
{
    static ArrayClock4 rTemp;
    return string_to_ArrayClock4(str, (void*)&rTemp, endptr);
}

int string_to_ArrayClock4(const char *str, void *pValue, char **endptr)
{
    int nRet = 0;
    skip_whitespace(str);
    if (pSimArrayClock4VTable != NULL) {
        nRet = string_to_VTable(str, pSimArrayClock4VTable, pValue, endptr);
    }
    if (nRet == 0) {
        nRet = pConverter->m_pfnStringToArray(str, pValue, &_Type_ClockStatus_Utils, 4, sizeof(ClockStatus), endptr);
    }
    return nRet;
}

int is_ArrayClock4_double_conversion_allowed()
{
    if (pSimArrayClock4VTable != NULL) {
        return is_VTable_double_conversion_allowed(pSimArrayClock4VTable);
    }
    return 0;
}

int ArrayClock4_to_double(const void *pValue, double *nValue)
{
    if (pSimArrayClock4VTable != NULL) {
        return VTable_to_double(pValue, pSimArrayClock4VTable, nValue);
    }
    return 0;
}

int is_ArrayClock4_long_conversion_allowed()
{
    if (pSimArrayClock4VTable != NULL) {
        return is_VTable_long_conversion_allowed(pSimArrayClock4VTable);
    }
    return 0;
}

int ArrayClock4_to_long(const void *pValue, long *nValue)
{
    if (pSimArrayClock4VTable != NULL) {
        return VTable_to_long(pValue, pSimArrayClock4VTable, nValue);
    }
    return 0;
}

void compare_ArrayClock4(int *pResult, const void *pValue1, const void *pValue2, SimTolerance *pTol, const char *pszPath, PFN_STR_LIST_APPEND pfnStrListAppend, void *pListErrPaths)
{
    int unitResult = 0;
    /* Customized comparison */
    if (pSimArrayClock4VTable != NULL
        && pSimArrayClock4VTable->m_version >= Scv612
        && pSimArrayClock4VTable->m_pfnCompare != NULL) {
        if (pSimArrayClock4VTable->m_version >= Scv65) {
            /* R15 and higher: VTable Compare function shall UPDATE *pResult global flag (*pResult |= SIM_CMP_RES_LT/...): */
            unitResult = pSimArrayClock4VTable->m_pfnCompare(pResult, pValue1, pValue2);
        } else {
            /* Before R15: VTable Compare function shall SET *pResult global flag (*pResult = -1/1/0): */
            pSimArrayClock4VTable->m_pfnCompare(&unitResult, pValue1, pValue2);
            updateCompareResult(unitResult, pResult);
        }
    } else {
        /* Predefined comparison */
        pConverter->m_pfnArrayComparison(pResult, pValue1, pValue2, 
                compare_ClockStatus, 4, sizeof(ClockStatus), pTol, pszPath, pfnStrListAppend, pListErrPaths);
    }
    if (unitResult != 0 && pfnStrListAppend != NULL && pszPath != NULL && *pszPath != 0 && pListErrPaths != NULL)
        pfnStrListAppend(pszPath, pListErrPaths);
}

int get_ArrayClock4_signature(PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    return pConverter->m_pfnGetArraySignature(get_ClockStatus_signature, 4, pfnStrAppend, pStrObj); 
}

int init_ArrayClock4(void *pValue)
{
    size_t i;
    for (i = 0; i < 4; i++)
        init_ClockStatus(&((ClockStatus*)pValue)[i]);
    return 1;
}

int release_ArrayClock4(void *pValue)
{
    UNUSED(pValue);
    return 1;
}

int copy_ArrayClock4(void *pToValue, const void *pFromValue)
{
    kcg_copy_ArrayClock4((ArrayClock4*)pToValue, (ArrayClock4*)pFromValue);
    return 1;
}

SimTypeUtils _Type_ArrayClock4_Utils = {
    ArrayClock4_to_string,
    check_ArrayClock4_string,
    string_to_ArrayClock4,
    is_ArrayClock4_double_conversion_allowed,
    ArrayClock4_to_double,
    is_ArrayClock4_long_conversion_allowed,
    ArrayClock4_to_long,
    compare_ArrayClock4,
    get_ArrayClock4_signature,
    init_ArrayClock4,
    release_ArrayClock4,
    copy_ArrayClock4,
    sizeof(ArrayClock4)
